...
In the previous articles in this series, we have dealt only with data that was local to the application, meaning that the whole lifecycle of our data was under control. The vast majority of the applications will have some interface with an external service, the app's REST API more often than not.

Today, we will discuss how we can handle data at the boundary of our application, making sure we have well formed objects of the proper type, whenever data comes into our Vue.js application from external sources.

## The problem

Whenever we reach out for external APIs, we cannot know for sure the shape of the data that will be delivered to us. We have to rely on the API's documentation, if available, to build our front-end app, but as JavaScript and JSON APIs don't enforce a particular schema on the data, we should keep some level of distrust on the provided data.

So, in order to increase our confidence in the data we are handling in our apps, we should establish a clear boundary between our app and the consumed APIs, making sure that all the data that comes in through this boundary is in fact in the shape we expect.
...
Our objective is to have a service layer that handles all external communication, making sure that the data that comes into our application is in a proper shape.
...
We have added the `id` field for the invoice, with type `number` or `null` (for when the invoice was still not created on the server side).

We now want to build a service layer that allows us to do CRUD operations in the invoices. Let's define the service skeleton now.
...
We are assuming a REST API accessible in the same origin as our own app, with all CRUD endpoints available.
...
In the [last article](), we have introduced a `Partial` type, for when we didn't have all the required attributes for a type. We can reuse it here to treat the data coming from the API as a possibly partial object of the type we want.
...
The `parse` function takes an unknown data object, that might also be undefined. We then build a valid Invoice object by giving it a null id by default and by parsing the other fields appropriately. For the `totalAmount` field, for example, we call the `Decimal` constructor to build a valid decimal, whereas the data coming from the API would most definitely not be a decimal object - it would be a string or a number, depending on how the data is serialized by the server.

We have delegated the User and Line Item handling to their own modules' parse function, which we didn't implement it. Let's do that now.
...
This is our final implementation of the Invoice service:
...
Notice how simple the service has become, as we are deferring the parsing logic to the Invoice module. The service responsibility is to talk to the API and then pipe the API's response to the parsing logic into the module. This will ensure that the return type of the service is well known.

## Handling mismatch of payload shapes

In the examples above, we made the assumption that the shape of the objects coming from the API are similar to the shape of the objects in our Vue.js application. If we control both sides of the equation, we can (and probably should) enforce that. But sometimes we either don't control the server side or we can't change it for whatever reason.
...
We don't need to export this interface to the rest of the application, nor declare it inside our Types namespace. It is intended to be used solely in the parse function. In the rest of the application we should use only the proper `Types.Product` type.
...
In the examples we built in this article, we are just ignoring the fact that the API didn't send a full object of the expected type. Sometimes that might be all we need to do. But in most occasions that is something we should be notified about and fix either our API (if that's something we control) or our parsing functions.

The `parse` functions we wrote are the perfect place to do that. If you are using something like [Sentry]() to collect front-end exceptions, then you can raise an exception if there is any missing field. Or you can use the [Capture Console]() integration and log an error and return a valid default (like we did in the examples) so that the application is still usable, even if there's some data missing.
...
We have created a clear boundary for our application, using services to convert data coming from external sources (such as our own backend API), making sure that the data coming into our app  will have a known shape.
...
The resulting service and parse functions are short, easy to reason about and easy to test, and they make us more confident that an unexpected response from the API will not break our front-end application.
